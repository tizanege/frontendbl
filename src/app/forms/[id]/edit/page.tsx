...
type FormStatus = "draft" | "published" | "active";
...
function useHistory<T>(initial: T) {
    ...
    // Force set present (for initial fetch)
    ...
    return { value: present, set: push, undo, redo, reset, canUndo: past.length > 0, canRedo: future.length > 0 };
}
...
export default function EditFormPage() {
    ...
    useEffect(() => {
        const fetchForm = async () => {
            try {
                ...
                setFormStatus(form.status === 'active' ? 'published' : (form.status || "draft"));
                if (form.schema?.fields) {
                    history.reset(form.schema.fields);
                }
                // Add current version to list
                setVersions([
                    { id: `v${form.version || 1}`, timestamp: new Date(form.updated_at), fieldCount: form.schema?.fields?.length || 0, status: form.status === 'active' ? 'published' : (form.status || "draft") }
                ]);
            } catch (err) {
    ...
    useEffect(() => {
        if (isFetching || (fields.length === 0 && !formName)) return;
        ...
        }, 5000); // 5s for edit mode to be less aggressive than 2s
        return () => { if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current); };
    }, [fields, formName, description, category, isFetching]);
    ...
    if (isFetching) return (
        <div className="h-screen flex items-center justify-center bg-slate-50">
            <div className="text-center">
                ...
                <p className="text-slate-500 font-bold">Synchronizing protocol...</p>
            </div>
        </div>
    );
            ...
            <div className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 shrink-0 z-50">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
                        ...
                        <button onClick={() => setFormStatus("published")}
                            className={`px-3 py-1 rounded-md text-[11px] font-bold transition-all ${formStatus === "published" || formStatus === "active" ? "bg-emerald-500 text-white shadow-sm" : "text-slate-400 hover:text-slate-600"}`}>
                            Published
                        </button>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button variant="ghost" size="sm" className="h-8 px-2 gap-1 rounded-lg text-slate-500">
                        <Link href={`/forms/${id}/submit`} target="_blank" className="flex items-center gap-1">
                ...
                <div className="w-56 bg-white border-r border-slate-200 flex flex-col shrink-0 overflow-hidden">
                    ...
                    <div className="p-3 border-t border-slate-100 bg-slate-50/50">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className={`text-[8px] font-bold px-2 py-0 rounded-md ${formStatus === "published" || formStatus === 'active' ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-400 border-slate-200"}`}>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-5">
                                {propertyTab === "general" && (
                                    <>
                                        ...
                                        {!["geotag", "image", "section", "checkbox", "signature", "logo"].includes(selectedField.type) && (
                                ...
                                {propertyTab === "validation" && (
                                    <>
                                        ...
                                        {["section", "image", "geotag", "signature", "logo"].includes(selectedField.type) && (
                            ...
                            {versions.length > 0 ? versions.map((v, i) => (
                                <div key={v.id} className={`p-3 rounded-xl border transition-all cursor-pointer ${i === 0 ? "border-blue-200 bg-blue-50/50" : "border-slate-100 hover:border-slate-200 hover:bg-slate-50"
                                    ...
                                    <div className="flex items-center justify-between mb-1">
                                        ...
                                        <Badge variant="outline" className={`text-[8px] font-bold px-1.5 py-0 rounded ${v.status === "published" || v.status === 'active' ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-400"
                                            }`}>
                                            {v.status}
                                        </Badge>
                                    </div>
